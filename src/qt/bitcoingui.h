...
class FiatUnitDisplayStatusBarControl;
...
private:
    ...
    FiatUnitDisplayStatusBarControl* fiatUnitDisplayControl = nullptr;
...
class FiatUnitDisplayStatusBarControl : public QLabel
{
    Q_OBJECT

public:
    explicit FiatUnitDisplayStatusBarControl(const PlatformStyle *platformStyle);
    /** Lets the control know about the Options Model (and its signals) */
    void setOptionsModel(OptionsModel *optionsModel);
...
private:
    ...
    /** Shows context menu with Display Unit options by the mouse coordinates */
    void onFiatDisplayUnitsClicked(const QPoint& point);
    /** Creates context menu, its actions, and wires up all the relevant signals for mouse events. */
    void createContextMenu();

private Q_SLOTS:
    /** When Display Units are changed on OptionsModel it will refresh the display text of the control on the status bar */
    void updateFiatDisplayUnit(int newUnits);
    /** Tells underlying optionsModel to update its current display unit. */
    void onMenuSelection(QAction* action);
};
